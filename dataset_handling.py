...
class DatasetHandler:
    ...
    def sample_dataset(self, dataset, sample_size, shuffle_data):
        """
        Samples a dataset to the specified sample size, optionally shuffling the data.
        """
        ...
        if shuffle_data:
            ...
            if sample_size:
                # Select the first 'sample_size' examples
                dataset = dataset.select(range(sample_size))
        else:
            if sample_size:
                # Select the first 'sample_size' examples without shuffling
                dataset = dataset.select(range(sample_size))
            else:
                # No sampling or shuffling
                pass
